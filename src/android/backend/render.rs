...
pub(crate) fn render_activity_windows(backend: &mut WaylandBackend) {
    ...
    for window_id in window_ids {
        ...
        let vk_rendered = try_render_vulkan_dmabuf(backend, window_id, &wl_surface, geo_offset, scale);
        ...
        render_gles_window(backend, window_id, &wl_surface, size, geo_offset, scale, center_y_physical, time);
    }
}

/// Try the Vulkan dmabuf render path for a window. Returns `true` if Vulkan
/// handled the frame (either rendered or determined no redraw was needed),
/// meaning the GLES fallback should be skipped.
fn try_render_vulkan_dmabuf(
    backend: &mut WaylandBackend,
    window_id: u32,
    wl_surface: &WlSurface,
    geo_offset: smithay::utils::Point<i32, smithay::utils::Logical>,
    scale: f64,
) -> bool {
...
/// Render a window using the GLES fallback path (for wl_shm clients).
/// Handles lazy EGL surface creation, popup compositing, GLES draw, swap,
/// frame callbacks, and metrics updates.
fn render_gles_window(
    backend: &mut WaylandBackend,
    window_id: u32,
    wl_surface: &WlSurface,
